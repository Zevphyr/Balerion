    ...
    async def g(self, ctx, *, query):
        ...
        if not embed_perms(ctx.message):
            ...
            async with self.bot.session.get(
                    "https://www.googleapis.com/customsearch/v1?q=" + urllib.parse.quote_plus(
        ...
        async with self.bot.session.get("https://www.googleapis.com/customsearch/v1?q=" + urllib.parse.quote_plus(
            ...
            if resp.status != 200:
                ...
                try:
                    ...
                    return await ctx.send(
                        self.bot.bot_prefix + 'There were no results to your search. Use more common search query or \
                        make sure you have image search enabled for your custom search engine.')
